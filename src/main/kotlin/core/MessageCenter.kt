...
object MessageCenter {
     ...
     */
    fun registerUser(params:JSONObject,session:Session? = null): JSONObject {
     ...
     */
    fun loginUser(params:JSONObject,session:Session? = null) : JSONObject {
     ...
     */
    fun updateUser(params:JSONObject,session:Session? = null): JSONObject {
            ...
            if (status_code != Users.UserUpdateResultCode.RESULT_OK) {
                ...
                if (params.contains("profile_image_checksum") && params.get("profile_image_checksum").toString().isEmpty()) {
                    ...
                    if (checksum>0) {
                        ...
                        this.file_requests.set(checksum, pending_request)
     ...
     */
    fun handleUserLoginRequests(action: Users.UserLoginAction,request:JSONObject,session:Session? = null):JSONObject {
        ...
        when(action) {
            Users.UserLoginAction.register_user -> {
                val result = registerUser(request,session)
            ...
            Users.UserLoginAction.login_user -> {
                val result = loginUser(request,session)
                if (result.contains("status")) {
                    result.set("request_id", request.get("request_id"))
                    response = result
                }
     ...
     * @return result of operation as JSON Object
     */
    fun handleUserAppRequests(action: Users.UserAppAction, request:JSONObject,session:Session? = null):JSONObject {
        ...
        when (action) {
            Users.UserAppAction.update_user -> {
                val result = updateUser(request,session)
                if (result.contains("status")) {
        ...
        if (message != null) {
            ...
            if (obj.containsKey("request_id") && !obj.get("request_id").toString().isEmpty()) {
                ...
                if (Users.UserLoginAction.Is(action)!=null) {
                    response = this.handleUserLoginRequests(Users.UserLoginAction.valueOf(action),obj,session)
                } else if (Users.UserAppAction.Is(action)!=null) {
                    response = this.handleUserAppRequests(Users.UserAppAction.valueOf(action),obj,session)
                } else if (!obj.containsKey("user_id") || !obj.containsKey("session_id") ||
                        obj["session_id"].toString().isEmpty() || obj["user_id"].toString().isEmpty()) {
                    system_error_response.set("status_code", MessageObjectResponseCodes.AUTHENTICATION_ERROR)
                    response = system_error_response
                } else {
                    system_error_response.set("status_code", MessageObjectResponseCodes.INTERNAL_ERROR)
